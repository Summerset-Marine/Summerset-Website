...
const PRODUCT_RECOMMENDATIONS = [
  {
    ...
    reason:
      "Heavy-duty permanent pier engineered for demanding water and ice conditions — built to stand year-round on Lake Mendota.",
  },
  {
...
export default function LakeMendotaPage() {
  ...
  useEffect(() => {
    ...
    sanityFetch<LakePageContent | null>(LAKE_PAGE_QUERY, { lakeSlug: "lake-mendota" })
      .then((data) => setContent(data))
      .catch(() => setContent(null));
    sanityFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "madison" })
      .then((data) => setProjects((data ?? []).filter((p) => p.lake === "lake-mendota")))
      .catch(() => setProjects([]));
    sanityFetch<Testimonial[]>(MARKET_TESTIMONIALS_QUERY, { market: "madison" })
      .then((data) =>
        setTestimonials(
          (data ?? []).filter(
            (t) =>
              t.lakeSlug === "lake-mendota" ||
              (!t.lakeSlug &&
                (!t.lakeLabel || t.lakeLabel.toLowerCase().includes("mendota"))),
          ),
        ),
      )
  ...
  const heroImage = content?.heroImageUrl ?? "/images/smc/wisconsin-seasonal-pier-lake-mendota-001.jpg";

  return (
    <Layout>
      <PageMeta
        title="Permanent Piers on Lake Mendota, WI | Summerset Marine"
        description="Summerset Marine installs luxury permanent piers and boat lifts on Lake Mendota. Expert waterfront contractors serving Madison-area property owners."
        canonical="https://summersetmarine.com/markets/madison/lake-mendota"
      />
      <JsonLd
        data={localBusinessSchema({
          ...
          marketSlug: "madison",
          lake: "Lake Mendota",
          coordinates: { latitude: "43.1067", longitude: "-89.4207" },
        })}
      />
      <JsonLd
        data={serviceSchema({
          name: "Permanent Pier Installation on Lake Mendota",
          market: "Madison",
          marketSlug: "madison",
          lake: "Lake Mendota",
        })}
      ...
      <div className="bg-brand-offwhite px-6 md:px-30 py-3 border-b border-brand-border">
        <div className="mx-auto max-w-content flex items-center gap-2">
          ...
          <Link href="/markets/madison" className="font-serif text-xs text-brand-black/45 hover:text-brand-gold transition-colors">Madison</Link>
          <span className="text-[11px] text-brand-black/30">›</span>
          <span className="font-serif text-xs text-brand-black">{content?.lakeName || "Lake Mendota"}</span>
        </div>
      </div>
      ...
      <section className="relative h-[75vh] min-h-[540px] overflow-hidden bg-brand-navy">
        <img 
          src={heroImage} 
          alt={content?.heroImageAlt ?? "Permanent pier on Lake Mendota"} 
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-30 pb-18 pointer-events-none">
          ...
          <p className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-500 font-serif text-lg leading-relaxed text-brand-offwhite/65 max-w-[540px] mb-9">
            Permanent Piers & Lifts on Lake Mendota
          </p>
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-700 flex flex-wrap gap-8 items-center pointer-events-auto">
            <Link href="/markets/madison/contact" className="inline-block px-10 py-3 border border-brand-gold font-serif text-[13px] tracking-[.16em] text-brand-gold uppercase hover:bg-brand-gold hover:text-white transition-colors">Request Consultation</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-22">
        <div className="mx-auto max-w-content grid md:grid-cols-[1fr_1px_1fr] gap-18 items-start">
          <div>
            ...
            <h2 className="font-serif text-[42px] font-normal leading-[1.1] text-brand-black mb-5">Summerset Marine on<br/><em className="italic">{content?.lakeName || "Lake Mendota"}.</em></h2>
            ...
            {content?.historyText ? (
              ...
              <ContentPlaceholder label="years of experience, notable projects, installed base on Lake Mendota" />
            )}
          </div>
          ...
          <div>
            ...
            {content?.lakeStats?.length ? (
              ...
              <ContentPlaceholder label="Lake Mendota lake stats (average depth, square acres)" className="mt-4" />
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 pb-22">
        <div className="mx-auto max-w-[800px] text-center mb-16">
          ...
          <h2 className="font-serif text-[36px] font-light text-brand-black mb-6">Building on {content?.lakeName || "Lake Mendota"}</h2>
        ...
        <div className="mx-auto max-w-content">
          <div className="flex justify-between items-end mb-13">
            <div>
              ...
              <h2 className="font-serif text-[46px] font-light text-brand-black italic m-0 leading-[1.05]">{content?.lakeName || "Lake Mendota"} Projects</h2>
            </div>
            <Link href="/markets/madison/projects" className="font-serif text-[12px] tracking-[.14em] uppercase text-brand-navy border-b border-brand-navy/50 pb-0.5 hover:text-brand-gold hover:border-brand-gold transition-colors">All Madison Projects →</Link>
          </div>
          ...
          {allProjects.length > 0 ? (
            ...
            <ContentPlaceholder label="Lake Mendota project gallery photos (from Sanity projects, lake = lake-mendota)" className="mt-8" />
          )}
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-24 border-t border-brand-border">
        <div className="mx-auto max-w-[720px] text-center">
          ...
          {testimonials.length > 0 ? (
            ...
            <ContentPlaceholder label="Lake Mendota customer testimonials (from Sanity testimonials, lake = lake-mendota)" className="mx-auto" />
          )}
        </div>
      </section>
