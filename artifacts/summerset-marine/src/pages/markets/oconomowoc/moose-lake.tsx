...
const PRODUCT_RECOMMENDATIONS = [
  {
    ...
    reason:
      "Heavy-duty permanent pier engineered for demanding water and ice conditions — built to stand year-round on Moose Lake.",
  },
  {
...
export default function MooseLakePage() {
  ...
  useEffect(() => {
    ...
    sanityFetch<LakePageContent | null>(LAKE_PAGE_QUERY, { lakeSlug: "moose-lake" })
      .then((data) => setContent(data))
      .catch(() => setContent(null));
    sanityFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "oconomowoc" })
      .then((data) => setProjects((data ?? []).filter((p) => p.lake === "moose-lake")))
      .catch(() => setProjects([]));
    sanityFetch<Testimonial[]>(MARKET_TESTIMONIALS_QUERY, { market: "oconomowoc" })
      .then((data) =>
        setTestimonials(
          (data ?? []).filter(
            (t) =>
              t.lakeSlug === "moose-lake" ||
              (!t.lakeSlug &&
                (!t.lakeLabel || t.lakeLabel.toLowerCase().includes("moose"))),
          ),
        ),
      )
  ...
  const heroImage = content?.heroImageUrl ?? "/images/smc/wisconsin-seasonal-pier-moose-lake-001.jpg";

  return (
    <Layout>
      <PageMeta
        title="Permanent Piers on Moose Lake, WI | Summerset Marine"
        description="Summerset Marine installs luxury permanent piers and boat lifts on Moose Lake. Expert waterfront contractors serving Lake Country property owners."
        canonical="https://summersetmarine.com/markets/oconomowoc/moose-lake"
      />
      <JsonLd
        data={localBusinessSchema({
          ...
          lake: "Moose Lake",
          coordinates: { latitude: "43.0525", longitude: "-88.3934" },
        })}
      />
      <JsonLd
        data={serviceSchema({
          name: "Permanent Pier Installation on Moose Lake",
          market: "Oconomowoc / Lake Country",
          marketSlug: "oconomowoc",
          lake: "Moose Lake",
        })}
      ...
      <div className="bg-brand-offwhite px-6 md:px-30 py-3 border-b border-brand-border">
        <div className="mx-auto max-w-content flex items-center gap-2">
          ...
          <span className="text-[11px] text-brand-black/30">›</span>
          <span className="font-serif text-xs text-brand-black">{content?.lakeName || "Moose Lake"}</span>
        </div>
      </div>
      ...
      <section className="relative h-[75vh] min-h-[540px] overflow-hidden bg-brand-navy">
        <img 
          src={heroImage} 
          alt={content?.heroImageAlt ?? "Permanent pier on Moose Lake"} 
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-30 pb-18 pointer-events-none">
          ...
          <p className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-500 font-serif text-lg leading-relaxed text-brand-offwhite/65 max-w-[540px] mb-9">
            Permanent Piers & Lifts on Moose Lake
          </p>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-22">
        <div className="mx-auto max-w-content grid md:grid-cols-[1fr_1px_1fr] gap-18 items-start">
          <div>
            ...
            <h2 className="font-serif text-[42px] font-normal leading-[1.1] text-brand-black mb-5">Summerset Marine on<br/><em className="italic">{content?.lakeName || "Moose Lake"}.</em></h2>
            ...
            {content?.historyText ? (
              ...
              <ContentPlaceholder label="years of experience, notable projects, installed base on Moose Lake" />
            )}
          </div>
          ...
          <div>
            ...
            {content?.lakeStats?.length ? (
              ...
              <ContentPlaceholder label="Moose Lake lake stats (average depth, square acres)" className="mt-4" />
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 pb-22">
        <div className="mx-auto max-w-[800px] text-center mb-16">
          ...
          <h2 className="font-serif text-[36px] font-light text-brand-black mb-6">Building on {content?.lakeName || "Moose Lake"}</h2>
        ...
        <div className="mx-auto max-w-content">
          <div className="flex justify-between items-end mb-13">
            <div>
              ...
              <h2 className="font-serif text-[46px] font-light text-brand-black italic m-0 leading-[1.05]">{content?.lakeName || "Moose Lake"} Projects</h2>
            </div>
          ...
          {allProjects.length > 0 ? (
            ...
            <ContentPlaceholder label="Moose Lake project gallery photos (from Sanity projects, lake = moose-lake)" className="mt-8" />
          )}
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-24 border-t border-brand-border">
        <div className="mx-auto max-w-[720px] text-center">
          ...
          {testimonials.length > 0 ? (
            ...
            <ContentPlaceholder label="Moose Lake customer testimonials (from Sanity testimonials, lake = moose-lake)" className="mx-auto" />
          )}
        </div>
      </section>
