...
const LAKE_FILTERS = ["All","Okauchee Lake","Lac La Belle","Nagawicka Lake","Beaver Lake"] as const;

const LAKE_LABELS: Record<string, string> = {
  "okauchee-lake": "Okauchee Lake",
  "lac-la-belle": "Lac La Belle",
  "nagawicka-lake": "Nagawicka Lake",
...
function matchesLake(project: Project, filter: string): boolean {
  ...
  if (filter === "Okauchee Lake") return lake === "okauchee-lake";
  if (filter === "Lac La Belle") return lake === "lac-la-belle";
  if (filter === "Nagawicka Lake") return lake === "nagawicka-lake";
  if (filter === "Beaver Lake") return lake === "beaver-lake";
  return false;
}

export default function OconomowocProjectsPage() {
  ...
  useEffect(() => {
    ...
    sanityLiveFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "oconomowoc" })
  ...
  return (
    <Layout>
      <PageMeta
        title="Pier & Lift Projects — Oconomowoc / Lake Country, WI | Summerset Marine Construction"
        description="Browse completed Summerset Marine Construction projects across Lake Country — Okauchee Lake, Lac La Belle, Nagawicka Lake, and Beaver Lake. Permanent piers, boat lifts, and waterfront systems."
        canonical="https://summersetmarine.com/markets/oconomowoc/projects"
      />
      <JsonLd
        data={serviceSchema({
        ...
        .map((p) => (
          <JsonLd
            ...
            data={imageObjectSchema({
              ...
              lake: LAKE_LABELS[p.lake ?? ""] ?? (p.lake ?? "Oconomowoc / Lake Country"),
      ...
      <section className="bg-brand-navy text-white">
        <div className="mx-auto max-w-content px-6 py-16">
          <h1 className="font-serif text-4xl md:text-5xl">Oconomowoc / Lake Country Projects</h1>
          <p className="mt-5 max-w-2xl text-lg text-white/85">
            Completed permanent piers, boat lifts, and waterfront systems built by Summerset
            Marine across the Oconomowoc / Lake Country market.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-content px-6 py-12">
        ...
        {projects === null ? (
          ...
          <div className="mt-10">
            ...
            <ContentPlaceholder
              label="Oconomowoc / Lake Country project photos, captions, and install years (from Sanity projects)"
              className="mt-8"
            />
          </div>
        ) : (
                  ...
                  <div className="p-5">
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded bg-brand-navy px-2 py-1 text-xs font-semibold uppercase tracking-wide text-white">
                        {LAKE_LABELS[project.lake ?? ""] ?? (project.lake ?? "Oconomowoc / Lake Country")}
                      </span>
                      {project.productType ? (
