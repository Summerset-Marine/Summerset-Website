...
const PRODUCT_RECOMMENDATIONS = [
  {
    ...
    reason:
      "Heavy-duty permanent pier engineered for demanding water and ice conditions — built to stand year-round on Beaver Lake.",
  },
  {
...
export default function BeaverLakePage() {
  ...
  useEffect(() => {
    ...
    sanityFetch<LakePageContent | null>(LAKE_PAGE_QUERY, { lakeSlug: "beaver-lake" })
      .then((data) => setContent(data))
      .catch(() => setContent(null));
    sanityFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "oconomowoc" })
      .then((data) => setProjects((data ?? []).filter((p) => p.lake === "beaver-lake")))
      .catch(() => setProjects([]));
  ...
  return (
    <Layout>
      ...
      <JsonLd
        data={serviceSchema({
          name: "Permanent Pier Installation on Beaver Lake",
          market: "Oconomowoc / Lake Country",
          marketSlug: "oconomowoc",
          lake: "Beaver Lake",
        })}
