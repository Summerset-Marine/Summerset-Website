...
export default function DoorCountyPage() {
  ...
  const heroImage = content?.heroImageUrl ?? "/images/smc/door-county-location-hero-001.jpg";
    ...
    <Layout>
      ...
      <div className="bg-brand-offwhite px-6 md:px-30 py-3 border-b border-brand-border">
        <div className="mx-auto max-w-content flex items-center gap-2">
          ...
          <span className="font-serif text-xs text-brand-black">{content?.marketName || "Door County"}</span>
        </div>
      </div>
      ...
      <section className="relative h-[75vh] min-h-[540px] overflow-hidden bg-brand-navy">
        <img 
          src={heroImage} 
          alt={content?.heroImageAlt ?? "Door County"} 
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-30 pb-18 pointer-events-none">
          ...
          <h1 className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-150 font-serif text-5xl md:text-[80px] font-light leading-none text-brand-offwhite italic mb-[18px] max-w-[700px]">
            {content?.marketName || "Door County"}
          </h1>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-22">
        <div className="mx-auto max-w-content grid md:grid-cols-[1fr_1px_1fr] gap-18 items-start">
          <div>
            ...
            <h2 className="font-serif text-[42px] font-normal leading-[1.1] text-brand-black mb-5">Building in<br/><em className="italic">{content?.marketName || "Door County"}.</em></h2>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 pb-22 border-t border-brand-border">
        <div className="mx-auto max-w-content">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-brand-border border border-brand-border">
            {[
              { name: "Green Bay", slug: "green-bay", desc: "Permanent piers, lifts, and waterfront systems." },
              { name: "Sturgeon Bay", slug: "sturgeon-bay", desc: "Permanent piers, lifts, and waterfront systems." },
              { name: "Lake Michigan", slug: "lake-michigan", desc: "Permanent piers, lifts, and waterfront systems." },
            ].map(lake => (
              <Link key={lake.slug} href={`/markets/${marketSlug}/${lake.slug}`} className="group block bg-white hover:border-brand-gold transition-colors cursor-pointer relative p-7">
                <div className="font-serif text-[11px] tracking-[.2em] uppercase text-brand-gold mb-2.5">Waterway</div>
