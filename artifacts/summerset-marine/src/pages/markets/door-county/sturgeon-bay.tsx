...
const PRODUCT_RECOMMENDATIONS = [
  {
    ...
    reason:
      "Heavy-duty permanent pier engineered for demanding water and ice conditions — built to stand year-round on Sturgeon Bay.",
  },
  {
...
export default function SturgeonBayPage() {
  ...
  useEffect(() => {
    ...
    sanityFetch<LakePageContent | null>(LAKE_PAGE_QUERY, { lakeSlug: "sturgeon-bay" })
      .then((data) => setContent(data))
      .catch(() => setContent(null));
    sanityFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "door-county" })
      .then((data) => setProjects((data ?? []).filter((p) => p.lake === "sturgeon-bay")))
      .catch(() => setProjects([]));
    sanityFetch<Testimonial[]>(MARKET_TESTIMONIALS_QUERY, { market: "door-county" })
      .then((data) =>
        setTestimonials(
          (data ?? []).filter(
            (t) =>
              t.lakeSlug === "sturgeon-bay" ||
              (!t.lakeSlug &&
                (!t.lakeLabel || t.lakeLabel.toLowerCase().includes("sturgeon"))),
          ),
        ),
      )
  ...
  const heroImage = content?.heroImageUrl ?? "/images/smc/door-county-sturgeon-bay-wisconsin-lake-001.jpg";
  ...
  return (
    <Layout>
      ...
      <JsonLd
        data={serviceSchema({
          name: "Permanent Pier Installation on Sturgeon Bay",
          market: "Door County",
          marketSlug: "door-county",
          lake: "Sturgeon Bay",
        })}
      ...
      <div className="bg-brand-offwhite px-6 md:px-30 py-3 border-b border-brand-border">
        <div className="mx-auto max-w-content flex items-center gap-2">
          ...
          <Link href="/markets/door-county" className="font-serif text-xs text-brand-black/45 hover:text-brand-gold transition-colors">Door County</Link>
          <span className="text-[11px] text-brand-black/30">›</span>
          <span className="font-serif text-xs text-brand-black">{content?.lakeName || "Sturgeon Bay"}</span>
        </div>
      </div>
      ...
      <section className="relative h-[75vh] min-h-[540px] overflow-hidden bg-brand-navy">
        <img 
          src={heroImage} 
          alt={content?.heroImageAlt ?? "Clear Wisconsin waterway in Door County"} 
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-30 pb-18 pointer-events-none">
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 font-serif text-[12px] tracking-[.28em] text-brand-gold uppercase mb-[18px]">Primary Waterway</div>
          ...
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-700 flex flex-wrap gap-8 items-center pointer-events-auto">
            <Link href="/markets/door-county/contact" className="inline-block px-10 py-3 border border-brand-gold font-serif text-[13px] tracking-[.16em] text-brand-gold uppercase hover:bg-brand-gold hover:text-white transition-colors">Request Consultation</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-22">
        <div className="mx-auto max-w-content grid md:grid-cols-[1fr_1px_1fr] gap-18 items-start">
          <div>
            ...
            <h2 className="font-serif text-[42px] font-normal leading-[1.1] text-brand-black mb-5">Summerset Marine on<br/><em className="italic">{content?.lakeName || "Sturgeon Bay"}.</em></h2>
            ...
            {content?.historyText ? (
              ...
              <ContentPlaceholder label="years of experience, notable projects, installed base on Sturgeon Bay" />
            )}
          </div>
          ...
          <div>
            ...
            {!content?.lakeCharacteristics && (
              <ContentPlaceholder label="Sturgeon Bay lake stats (average depth, square acres)" className="mt-4" />
            )}
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 pb-22">
        <div className="mx-auto max-w-[800px] text-center mb-16">
          ...
          <h2 className="font-serif text-[36px] font-light text-brand-black mb-6">Building on {content?.lakeName || "Sturgeon Bay"}</h2>
        ...
        <div className="mx-auto max-w-content">
          <div className="flex justify-between items-end mb-13">
            <div>
              ...
              <h2 className="font-serif text-[46px] font-light text-brand-black italic m-0 leading-[1.05]">{content?.lakeName || "Sturgeon Bay"} Projects</h2>
            </div>
            <Link href="/markets/door-county/projects" className="font-serif text-[12px] tracking-[.14em] uppercase text-brand-navy border-b border-brand-navy/50 pb-0.5 hover:text-brand-gold hover:border-brand-gold transition-colors">All Door County Projects →</Link>
          </div>
          ...
          {projects.length > 0 ? (
            ...
            <ContentPlaceholder label="Sturgeon Bay project gallery photos (from Sanity projects, lake = sturgeon-bay)" className="mt-8" />
          )}
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-24 border-t border-brand-border">
        <div className="mx-auto max-w-[720px] text-center">
          ...
          {testimonials.length > 0 ? (
            ...
            <ContentPlaceholder label="Sturgeon Bay customer testimonials (from Sanity testimonials, lake = sturgeon-bay)" className="mx-auto" />
          )}
        </div>
      </section>
      ...
      <CTABlock
        variant="dark"
        headline="Ready to build on Sturgeon Bay?"
