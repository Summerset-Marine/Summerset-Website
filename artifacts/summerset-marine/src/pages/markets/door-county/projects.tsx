...
const LAKE_FILTERS = ["All","Green Bay","Sturgeon Bay","Lake Michigan — Door County"] as const;

const LAKE_LABELS: Record<string, string> = {
  "green-bay": "Green Bay",
  "sturgeon-bay": "Sturgeon Bay",
  "lake-michigan": "Lake Michigan — Door County",
};
...
function matchesLake(project: Project, filter: string): boolean {
  ...
  if (filter === "Green Bay") return lake === "green-bay";
  if (filter === "Sturgeon Bay") return lake === "sturgeon-bay";
  if (filter === "Lake Michigan — Door County") return lake === "lake-michigan";
  return false;
}

export default function DoorCountyProjectsPage() {
  ...
  useEffect(() => {
    ...
    sanityLiveFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "door-county" })
    ...
    <Layout>
        ...
        .map((p) => (
          <JsonLd
            ...
            data={imageObjectSchema({
              ...
              lake: LAKE_LABELS[p.lake ?? ""] ?? (p.lake ?? "Door County"),
      ...
      <section className="bg-brand-navy text-white">
        <div className="mx-auto max-w-content px-6 py-16">
          <h1 className="font-serif text-4xl md:text-5xl">Door County Projects</h1>
          <p className="mt-5 max-w-2xl text-lg text-white/85">
            Completed permanent piers, boat lifts, and waterfront systems built by Summerset
            Marine across the Door County market.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-content px-6 py-12">
        ...
        {projects === null ? (
          ...
          <div className="mt-10">
            ...
            <ContentPlaceholder
              label="Door County project photos, captions, and install years (from Sanity projects)"
              className="mt-8"
            />
          </div>
        ) : (
                  ...
                  <div className="p-5">
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded bg-brand-navy px-2 py-1 text-xs font-semibold uppercase tracking-wide text-white">
                        {LAKE_LABELS[project.lake ?? ""] ?? (project.lake ?? "Door County")}
                      </span>
                      {project.productType ? (
