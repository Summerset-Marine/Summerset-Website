...
const PRODUCT_RECOMMENDATIONS = [
  {
    ...
    reason:
      "Best for Geneva Lake's deep-water conditions — oversized pilings driven deep into the lakebed stand firm against wave action and ice movement.",
  },
  {
  ...
  },
  {
    ...
    reason:
      "Flexible placement and compatibility with existing waterfront setups — a strong fit for varied Geneva Lake shorelines.",
  },
];

export default function GenevaLakePage() {
  ...
  useEffect(() => {
    ...
    sanityFetch<LakePageContent | null>(LAKE_PAGE_QUERY, { lakeSlug: "geneva-lake" })
      .then((data) => setContent(data))
      .catch(() => setContent(null));
    sanityFetch<Project[]>(MARKET_PROJECTS_QUERY, { market: "lake-geneva" })
      .then((data) => setProjects((data ?? []).filter((p) => p.lake === "geneva-lake")))
      .catch(() => setProjects([]));
    sanityFetch<Testimonial[]>(MARKET_TESTIMONIALS_QUERY, { market: "lake-geneva" })
      .then((data) =>
        setTestimonials(
          (data ?? []).filter(
            (t) =>
              t.lakeSlug === "geneva-lake" ||
              (!t.lakeSlug &&
                (!t.lakeLabel || t.lakeLabel.toLowerCase().includes("geneva"))),
          ),
        ),
      )
  ...
  return (
    <Layout>
      ...
      <JsonLd
        data={serviceSchema({
          name: "Permanent Pier Installation on Geneva Lake",
          market: "Lake Geneva",
          marketSlug: "lake-geneva",
          lake: "Geneva Lake",
        })}
      ...
      <div className="bg-brand-offwhite px-6 md:px-30 py-3 border-b border-brand-border">
        <div className="mx-auto max-w-content flex items-center gap-2">
          ...
          <Link href="/markets/lake-geneva" className="font-serif text-xs text-brand-black/45 hover:text-brand-gold transition-colors">Lake Geneva</Link>
          <span className="text-[11px] text-brand-black/30">›</span>
          <span className="font-serif text-xs text-brand-black">{content?.lakeName || "Geneva Lake"}</span>
        </div>
      </div>
      ...
      <section className="relative h-[75vh] min-h-[540px] overflow-hidden bg-brand-navy">
        <img 
          src={heroImage} 
          alt={content?.heroImageAlt ?? "Permanent pier aerial view on Geneva Lake"} 
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-30 pb-18 pointer-events-none">
          ...
          <p className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-500 font-serif text-lg leading-relaxed text-brand-offwhite/65 max-w-[540px] mb-9">
            Permanent Piers & Lifts on Geneva Lake
          </p>
          <div className="animate-in slide-in-from-bottom-4 fade-in duration-700 delay-700 flex flex-wrap gap-8 items-center pointer-events-auto">
            <Link href="/markets/lake-geneva/contact" className="inline-block px-10 py-3 border border-brand-gold font-serif text-[13px] tracking-[.16em] text-brand-gold uppercase hover:bg-brand-gold hover:text-white transition-colors">Request Consultation</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-22">
        <div className="mx-auto max-w-content grid md:grid-cols-[1fr_1px_1fr] gap-18 items-start">
          <div>
            ...
            <h2 className="font-serif text-[42px] font-normal leading-[1.1] text-brand-black mb-5">Summerset Marine on<br/><em className="italic">{content?.lakeName || "Geneva Lake"}.</em></h2>
            ...
            {content?.historyText ? (
              ...
              <ContentPlaceholder label="years of experience, notable projects, installed base on Geneva Lake" />
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-baseline py-5 border-b border-brand-border">
                ...
                <span className="font-serif text-[32px] font-light text-brand-navy tabular-nums">61 Feet</span>
              </div>
              <div className="flex justify-between items-baseline py-5 border-b border-brand-border">
                <span className="font-serif text-[14px] text-brand-black/60">Square Acres</span>
                <span className="font-serif text-[32px] font-light text-brand-navy tabular-nums">5,401</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 pb-22">
        <div className="mx-auto max-w-[800px] text-center mb-16">
          ...
          <h2 className="font-serif text-[36px] font-light text-brand-black mb-6">Building on {content?.lakeName || "Geneva Lake"}</h2>
        ...
        <div className="mx-auto max-w-content">
          <div className="flex justify-between items-end mb-13">
            <div>
              ...
              <h2 className="font-serif text-[46px] font-light text-brand-black italic m-0 leading-[1.05]">{content?.lakeName || "Geneva Lake"} Projects</h2>
            </div>
          ...
          {allProjects.length > 0 ? (
            ...
            <ContentPlaceholder label="Geneva Lake project gallery photos (from Sanity projects, lake = geneva-lake)" className="mt-8" />
          )}
        </div>
      </section>
      ...
      <section className="bg-brand-offwhite px-6 md:px-30 py-24 border-t border-brand-border">
        <div className="mx-auto max-w-[720px] text-center">
          ...
          {testimonials.length > 0 ? (
            ...
            <ContentPlaceholder label="Geneva Lake customer testimonials (from Sanity testimonials, lake = geneva-lake)" className="mx-auto" />
          )}
        </div>
      </section>
      ...
      <CTABlock
        ...
        primaryCta={{
          label: "Request a Consultation on Geneva Lake",
